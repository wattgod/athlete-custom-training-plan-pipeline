...
def _build_strength_session(
    *, week_num: int, phase: str, race_name: str, weekday: date, used_titles: set,
    ...
    program = WorkoutLibrary.get_strength_workout(
        week_num=week_num, session_num=1, equipment_tier="full-gym",
        phase=phase, is_recovery_week=False, is_masters=False,
    )
...
def _week_coach_note(sessions: List[Dict[str, Any]], race_name: str) -> str:
    plan_ir = {
        "athlete": {"id": "preview"},
    ...
    target_title = f"Week {_WEEK_NUMBER}: Build"
    for note in notes:
...
def generate_preview_source(normalized_request: Mapping[str, Any]) -> Dict[str, Any]:
    """Run the real Motoren engine in-memory and return a preview_contract
    ``source`` dict for the given normalized ``training-plan-preview-request/v1``
    payload (see ``preview_contract.normalize_request``'s output shape).
