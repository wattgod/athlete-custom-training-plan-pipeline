...
| Hourly | `state-audit.yml` calls Railway's authenticated `POST /api/cron/state-audit`; the endpoint runs `audit_fulfillment_states.py` against the live persistent order root | Pipeline operator | A critical finding returns HTTP 500 and makes the workflow red. Investigate authenticated state before changing it. |
