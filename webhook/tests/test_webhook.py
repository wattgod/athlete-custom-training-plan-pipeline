...
class TestPipelineFailureAthleteEmail:
    """A pipeline failure must email the ATHLETE (not just the coach) so a paid
    order never sits silently — Taylor Foster (Big Sugar Gravel, 2026-06-22)
    failed and only the coach was notified."""

    def _details(self):
        return {'name': 'Taylor Foster', 'email': 'taylor@example.com',
                'race_name': 'Big Sugar Gravel', 'order_id': 'cs_live_x',
                'athlete_id': 'taylor_foster'}

    def test_failed_order_emails_the_athlete(self):
        from app import _notify_new_order
        with patch('app._send_email') as mock_send:
            mock_send.return_value = True
            with patch.dict(os.environ, {'NOTIFICATION_EMAIL': 'coach@gg.com',
                                         'RESEND_API_KEY': 'x'}):
                _notify_new_order('training_plan_FAILED', self._details())
        recipients = [c.args[0] for c in mock_send.call_args_list]
        assert 'taylor@example.com' in recipients  # athlete got an email

    def test_successful_order_does_not_send_delay_email(self):
        from app import _notify_new_order
        with patch('app._send_email') as mock_send:
            mock_send.return_value = True
            with patch.dict(os.environ, {'NOTIFICATION_EMAIL': 'coach@gg.com',
                ...
                d = self._details()
                d['download_token'] = 't'
                _notify_new_order('training_plan', d)
        recipients = [c.args[0] for c in mock_send.call_args_list]
        assert 'taylor@example.com' not in recipients  # only the coach

    def test_delay_email_subject_and_no_throw_without_email(self):
        from app import _send_pipeline_delay_email
        # missing athlete email must not throw, must return False
        assert _send_pipeline_delay_email({'name': 'X'}) is False
        with patch('app._send_email') as mock_send:
            mock_send.return_value = True
            assert _send_pipeline_delay_email(self._details()) is True
