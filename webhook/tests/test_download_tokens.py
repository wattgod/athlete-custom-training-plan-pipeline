...
def test_typed_claims_round_trip():
...
def test_cross_scope_reuse_is_rejected(field, override):
    with pytest.raises(DownloadTokenError, match='mismatch|audience'):
        _verify(_token(), **{field: override})


def test_expiry_and_max_lifetime_are_enforced():
...
def test_unknown_artifact_and_unknown_kid_fail_closed(monkeypatch):
...
def test_per_link_and_per_key_revocation(tmp_path):
...
def test_customer_key_cannot_sign_coach_artifact():
